...
because they're already authenticated to Clement's workspace. This module
defines:
...
def build_page_properties(brief: dict[str, Any]) -> dict[str, Any]:
    ...
    """
    # Properties stay SHORT and scannable (this is the table view).
    # All long creative content lives in the page body (build_page_content).
    props: dict[str, Any] = {
        "Trend": brief["trend"],
...
def build_page_content(brief: dict[str, Any]) -> str:
    ...
    if brief.get("my_take"):
        lines += ["", "## my take (notes for clement)", "", brief["my_take"].strip()]
